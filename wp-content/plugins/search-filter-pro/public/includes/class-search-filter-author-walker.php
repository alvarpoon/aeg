...
class Search_Filter_Author_Walker {
	...
	function wp_authors($type, $args = '') {
		...
		foreach ( $authors as $author_id ) {
			...
			{
				$input_id = SF_INPUT_ID_PRE.sanitize_html_class($args['name'].esc_attr($author->ID));
				
				$link = '<input type="radio" name="'.$args['name'].'[]" id="'.$input_id.'" value="'.esc_attr($author->ID).'"'.$checked.'><label for="'.$input_id.'">'.esc_html($name);
			}
			else if($this->type=="checkbox")
			{
				$input_id = SF_INPUT_ID_PRE.sanitize_html_class($args['name'].esc_attr($author->ID));
				$link = '<input type="checkbox" name="'.$args['name'].'[]" id="'.$input_id.'" value="'.esc_attr($author->ID).'"'.$checked.'><label for="'.$input_id.'">'.esc_html($name);
			}
